...
const keyword = ['global', 'function', 'probe', 'while', 'for', 'foreach', 'in', 'limit', 'for', 'break', 'continue', 'return', 'next', 'delete', 'try', 'catch']
	.map(i => {
		const j = new vscode.CompletionItem(i);
...
export function provideCompletionItemsInLocal(
	...
	for (const i of variables) {
		if (!added[i.name]) {
			added[i.name] = true;
			const completionItem = new vscode.CompletionItem(i.name);
...
export function provideCompletionItemsAfterProbe(
	...
	const input = document.lineAt(position).text.substring(0, position.character);

	let afterProbe: vscode.CompletionItem[] = [];

	const syscall = new vscode.CompletionItem('syscall');
	...
	const kernel = new vscode.CompletionItem('kernel');
	...
	const vfs = new vscode.CompletionItem('vfs');
	...
	const process = new vscode.CompletionItem('process');
	...
	const begin = new vscode.CompletionItem('begin');
	const end = new vscode.CompletionItem('end');
	const timer = new vscode.CompletionItem('timer');
...
export default function setupAutoCompletion(context: vscode.ExtensionContext) {
	const autoCompletion = vscode.languages.registerCompletionItemProvider('systemtap', { provideCompletionItems });
	const autoCompletionMacro = vscode.languages.registerCompletionItemProvider('systemtap', { provideCompletionItems: provideCompletionItemsMacro }, '@');
	const autoCompletionAfterProbe = vscode.languages.registerCompletionItemProvider('systemtap', { provideCompletionItems: provideCompletionItemsAfterProbe }, ' ', '.');
